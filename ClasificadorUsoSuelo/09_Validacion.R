...
PuntosVal <- st_transform(st_read("SHP/0_Validacion/RabdomSample1000.gpkg.shp"), 
                          crs=crs(MET_sf))
...
tm_shape(MET_sf) + tm_polygons("DN", 
                               border.alpha = .5, 
                               palette="Reds", 
                               legend.show = FALSE,
                               max.categories = 11) + #, palette=cols
  tm_format("World", 
            title="Cobertura Vegetal y Uso del Suelo \n Linares, Nuevo México") #, col = cols


#Union puntos de validación y clasificación
DatosVal <- st_join(PuntosVal, 
                    MET_sf, 
                    join = st_intersects) %>%
            select(c("class", "DN")) %>% 
            na.omit()

matrizConfucion <- data.frame(matrix(table(DatosVal$class,DatosVal$DN), 
                                     nrow = 11, 
                                     ncol = 11))
