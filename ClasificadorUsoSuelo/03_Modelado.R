...
# library(mlr)
# 
# #Create a classification task for learning on valuetable
# task <- makeClassifTask(data=valuetable, target="class")
# 
...
# predictions_mlr <- predict(NB_mlr, newdata = valuetable[,1:12])
# 
# #Confusion matrix 
# #table(predictions_mlr[,1], valuetable$class)
# 
# predNB <- predict(covs, model=NB_mlr, na.rm=TRUE, 
#                   progress="text", 
#                   file="TIFF/NB_class.tiff_Entrenamiento5_ANIR2", 
#                   overwrite=TRUE)
# 
# graphics.off()
# plot(predNB, col=cols)
# plot(Linares, add = TRUE)
...
# library(kernlab)
# 
# svm <- ksvm(class ~ ., data = valuetable)
# predLC_SVM <- predict(covs, model=svm, na.rm=TRUE, progress="text", 
#                       file="TIFF/SVM_class_RADAR.tiff", overwrite=TRUE)
# 
# plot(predLC_SVM, col=cols)
# plot(Linares, add=TRUE)
# 
# #Support Vector Machine
# library(kernlab)
# 
# GLM <- glm(as.numeric(class) ~ Variables.1 + Variables.2 + Variables.3 + 
#              Variables.4 +  Variables.5 +  Variables.6 +  
#              Variables.7 + Variables.8 + Variables.9 + Variables.10, data = valuetable)
# predLC_GLM <- predict(covs, model=GLM, na.rm=TRUE, progress="text", file="TIFF/GLM_class_B5.tiff", overwrite=TRUE)
# 
# plot(predLC_GLM, col=cols)
# plot(Linares, add=TRUE)
# 
# 
# #Árbol de decisión
...
# 
# # grow tree 
# fit <- rpart(class ~ ., data = valuetable)#, method="class"
# summary(fit)
# #Predict Output 
# predLC_Arbol <- predict(covs, model=fit, na.rm=TRUE, progress="text", 
#                         file="TIFF/Arbol_class_TreeCover_B5.tiff", overwrite=TRUE)
# plot(predLC_Arbol, col=cols)
